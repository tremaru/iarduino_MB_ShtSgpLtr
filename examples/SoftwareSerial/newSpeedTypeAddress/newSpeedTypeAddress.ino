...
// На шине должен быть только один модуль климатических датчиков.                                  //
...
//   Находим текущий адрес модуля климатических датчиков, скорость и тип протокола Modbus:         //
...
//   - если скорость известна, то строки с другими скоростями можно удалить.                       //
             ...
             Serial.print("Поиск модуля климатических датчиков...");                               //
     ...
     if(!f){ rs485.begin( 57600); wait_rs485(); Serial.print('.'); f=sensor.begin(); }             //   Инициируем работу с модулем на скорости  57600, без указания адреса (адрес будет найден автоматически).
     if(!f){ rs485.begin(115200); wait_rs485(); Serial.print('.'); f=sensor.begin(); }             //   Инициируем работу с модулем на скорости 115200, без указания адреса (адрес будет найден автоматически).
//   Выводим результат обнаружения модуля климатических датчиков:                                  //
     if(!f){ Serial.println("\r\nМодуль климатических датчиков не найден"); return; }              //
     else  { Serial.println("\r\nМодуль климатических датчиков найден"   );         }              //
//   Меняем адрес модуля климатических датчиков:                                                   //
     f =     sensor.changeID(newAddress);                                                          //
     if(!f){ Serial.println("Адрес не изменён"); return; }                                         //
//   Устанавливаем новую скорость передачи данных модуля климатических датчиков:                   //
...
//   Подтверждаем изменение скорости передачи данных модуля климатических датчиков:                //
...
//   Устанавливаем новый тип протокола модуля климатических датчиков:                              //
...
//   Подтверждаем изменение типа протокола модуля климатических датчиков:                          //
...
//   Выводим сообшение об успешном изменении адреса, скорости и типа протокола:                    //
             ...
             Serial.println((String) "Адрес    = "+sensor.getID()                        );        //   Выводим адрес модуля климатических датчиков.
             Serial.println((String) "Версия   = "+sensor.getVersion()                   );        //   Выводим версию прошивки модуля климатических датчиков.
}                                                                                                  //
                                                                                                   //
void loop(){                                                                                       //
